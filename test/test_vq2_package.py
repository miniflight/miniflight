...
from target.aigp import extract_vq2, runtime
...
class VQ2PackageTest(unittest.TestCase):
    def setUp(self):
        ...
        for name in extract_vq2.CONFIG:
            (self.base / "vq2" / name).write_text(f"configuration {name}")
    ...
    def archive(self):
        ...
        self.part = self.base / "vq2-unlocked.tar.gz.part-aa"
    ...
    def test_uses_shared_installer_with_vq2_metadata(self):
        ...
        config = {Path("vq2") / name: path for name, path in extract_vq2.CONFIG.items()}
    ...
    def test_install_reuse_and_configuration_refresh(self):
        ...
        for name in extract_vq2.CONFIG:
            (self.base / "vq2" / name).write_text(f"updated {name}")
        self.part.unlink()
    ...
    def test_direct_execution_reuses_installed_package(self):
        ...
        self.part.unlink()
        result = subprocess.run([sys.executable, str(self.base / "extract_vq2.py")],
